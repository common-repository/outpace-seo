 ...
 * Meta Boxes setup
 */
if (!class_exists('Outpaceseo_Target_Rule_Fields')) {

    /**
     * Meta Boxes setup
     */
    class Outpaceseo_Target_Rule_Fields
    {
        ...
        public function __construct()
        {
            ...
            add_action('wp_ajax_op_get_posts_by_query', array($this, 'op_get_posts_by_query'));
        }
        ...
        public static function get_location_selections()
        {
            ...
            $post_types = apply_filters('op_location_rule_post_types', array_merge($post_types, $custom_post_type));

            $selection_options = array(
                'basic' => array(
                    'label' => __('Basic', 'outpaceseo'),
                    'value' => array(
                        'basic-singulars' => __('All Singulars', 'outpaceseo'),
                        'special-front'   => __('Front Page', 'outpaceseo'),
                    ),
                ),
            );
            ...
            $selection_options['specific-target'] = array(
                'label' => __('Specific Target', 'outpaceseo'),
                'value' => array(
                    'specifics' => __('Specific Pages / Posts / Archives etc.', 'outpaceseo'),
                ),
            );
        ...
        public static function get_user_selections()
        {
            $selection_options = array(
                'basic'    => array(
                    'label' => __('Basic', 'outpaceseo'),
                    'value' => array(
                        'all'        => __('All', 'outpaceseo'),
                        'logged-in'  => __('Logged In', 'outpaceseo'),
                        'logged-out' => __('Logged Out', 'outpaceseo'),
                    ),
                ),

                'advanced' => array(
                    'label' => __('Advanced', 'outpaceseo'),
                    'value' => array(),
                ),
            );
         ...
         */
        public function op_get_posts_by_query()
        {
         ...
         * This is only run from op_get_posts_by_query()
         *
        ...
        public function admin_styles()
        {
            ...
            $enqueue_admin_script  = self::outpaceseo_enqueue_admin_script();
            ...
            if (true === $enqueue_admin_script || 'outpaceseo_schema' === $schema_post_type_name) {
                ...
                wp_enqueue_script('op-target-rule-select2');
                wp_enqueue_script('op-target-rule');
                wp_enqueue_script('op-user-role');
            }
        }
         ...
         */
        public static function outpaceseo_enqueue_admin_script()
        {
            global $pagenow;
        ...
        public static function target_rule_settings_field($name, $settings, $value)
        {
            ...
            $add_rule_label = isset($settings['add_rule_label']) ? $settings['add_rule_label'] : __('Add Rule', 'outpaceseo');
            $saved_values   = $value;
            $output         = '';
            ...
            /* WP Template Format */
            $output .= '<script type="text/html" id="tmpl-op-target-rule-' . $rule_type . '-condition">';
            $output .= '<div class="op-target-rule-condition op-target-rule-{{data.id}}" data-rule="{{data.id}}" >';
            ...
            $output .= '<select name="' . esc_attr($input_name) . '[rule][{{data.id}}]" class="target_rule-condition form-control op-input">';
            $output .= '<option value="">' . __('Select Target Rule', 'outpaceseo') . '</option>';
            ...
            $output .= '</div> <!-- op-target-rule-condition -->';

            /* Specific page selection */
            $output .= '<div class="target_rule-specific-page-wrap" style="display:none">';
            $output .= '<select name="' . esc_attr($input_name) . '[specific][]" class="target-rule-select2 target_rule-specific-page form-control op-input" multiple="multiple">';
            $output .= '</select>';
            $output .= '</div>';
            ...
            /* Wrapper Start */
            $output .= '<div class="op-target-rule-wrapper op-target-rule-' . $rule_type . '-on-wrap" data-type="' . $rule_type . '">';
            $output .= '<div class="op-target-rule-selector-wrapper op-target-rule-' . $rule_type . '-on">';
        ...
        public static function get_post_target_rule_options($post_type, $taxonomies)
        {
            ...
            $all_posts                          = sprintf(__('All %s', 'outpaceseo'), $post_label);
            $post_option[$post_name . '|all'] = $all_posts;

            if ('pages' !== $post_key) {
                /* translators: %s post label */
                $all_archive = sprintf(__('All %s Archive', 'outpaceseo'), $post_label);
            }
            ...
            foreach ($taxonomies as $taxonomy) {
                ...
                $tax_archive = sprintf(__('All %s Archive', 'outpaceseo'), $tax_label);
            }
        ...
        public static function generate_target_rule_selector($type, $selection_options, $input_name, $saved_values, $add_rule_label)
        {
            ...
            foreach ($saved_values['rule'] as $index => $data) {
                ...
                $output .= '<div class="op-target-rule-condition op-target-rule-' . $index . '" data-rule="' . $index . '" >';
                /* Condition Selection */
                $output .= '<div class="target_rule-condition-wrap" >';
                $output .= '<select name="' . esc_attr($input_name) . '[rule][' . $index . ']" class="target_rule-condition form-control op-input">';
                $output .= '<option value="">' . __('Select Target Rule', 'outpaceseo') . '</option>';
        ...
        public function get_current_layout($layout_id, $option)
        {
            ...
            $current_layout = apply_filters('op_target_page_settings', $current_layout, $layout_id);
        ...
        public static function target_user_role_settings_field($name, $settings, $value)
        {
            ...
            /* WP Template Format */
            $output         .= '<script type="text/html" id="tmpl-op-user-role-condition">';
            $output     .= '<div class="op-user-role-condition op-user-role-{{data.id}}" data-rule="{{data.id}}" >';
            $output .= '<span class="user_role-condition-delete dashicons dashicons-dismiss"></span>';
            /* Condition Selection */
            $output     .= '<div class="user_role-condition-wrap" >';
            $output .= '<select name="' . esc_attr($input_name) . '[{{data.id}}]" class="user_role-condition form-control op-input select select2-class">';
            $output .= '<option value="">' . __('Select Target Rule', 'outpaceseo') . '</option>';
            ...
            $output .= '</select>';
            $output     .= '</div>';
            $output         .= '</div> <!-- op-user-role-condition -->';
            $output             .= '</script>';
            ...
            $output         .= '<div class="op-user-role-wrapper op-user-role-display-on-wrap" data-type="display">';
            $output     .= '<div class="op-user-role-selector-wrapper op-user-role-display-on">';
            $output .= '<div class="user_role-builder-wrap">';
            foreach ($saved_values as $index => $data) {
                $output     .= '<div class="op-user-role-condition op-user-role-' . $index . '" data-rule="' . $index . '" >';
                $output .= '<span class="user_role-condition-delete dashicons dashicons-dismiss"></span>';
                $output     .= '<div class="user_role-condition-wrap" >';
                $output .= '<select name="' . esc_attr($input_name) . '[' . $index . ']" class="user_role-condition form-control op-input select select2-class">';
                $output .= '<option value="">' . __('Select Target Rule', 'outpaceseo') . '</option>';
        ...
        public function get_posts_by_conditions($post_type, $option, $old_post_id = false)
        {
            ...
            if (false === $meta_header) {
                            ...
                            foreach ($taxonomy as $tax_slug) {
                                ...
                                if (!empty($tax_terms)) {
                                    foreach ($tax_terms as $tax_term) {
                                        ...
                                        if (0 !== $tax_term->parent && true === apply_filters('op_target_rules_include_parent_taxanomy', false)) {
                ...
                $query     = apply_filters('op_target_rules_main_query', $query);
                $meta_args = apply_filters('op_target_rules_meta_args', $meta_args);

                $posts = $wpdb->get_results($query . ' AND (' . $meta_args . ')' . apply_filters('op_target_rules_orderby', $orderby)); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
            ...
            if (!empty($already_set_rule)) {
                add_action(
                    ...
                    function () use ($already_set_rule) {
                        ...
                        $notice = sprintf(__('The same display setting does already exist in %1$s %2$s.', 'outpaceseo'), $rule_set_titles, $schema_str);
 ...
 */
Outpaceseo_Target_Rule_Fields::get_instance();
